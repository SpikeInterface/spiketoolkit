...
class HerdingspikesSorter(BaseSorter):
    ...
    def _setup_recording(self, recording, output_folder):
        ...
        self.Probe = hs.probe.RecordingExtractor(
            recording,
            masked_channels=self.params['probe_masked_channels'],
            inner_radius=self.params['probe_inner_radius'],
            neighbor_radius=self.params['probe_neighbor_radius'],
            event_length=self.params['probe_event_length'],
            peak_jitter=self.params['probe_peak_jitter'])
    ...
    def _run(self, recording, output_folder):
        ...
        self.H = hs.HSDetection(
            ...
            to_localize=True,
            num_com_centers=self.params['num_com_centers'],
            maa=self.params['maa'],
            ahpthr=self.params['ahpthr'],
            out_file_name=self.params['out_file_name'],
            decay_filtering=self.params['decay_filtering'],
            save_all=self.params['save_all'],
            amp_evaluation_time=self.params['amp_evaluation_time'],
            spk_evaluation_time=self.params['spk_evaluation_time']
        )
        ...
        if(not self.H.spikes.empty):
            ...
            self.C.ShapePCA(pca_ncomponents=self.params['pca_ncomponents'],
                            pca_whiten=self.params['pca_whiten'])
