...
def compute_unit_SNR(recording, sorting, unit_ids=None, save_as_property=True):
    ...
    if unit_ids is not None:
        templates = st.postprocessing_tools.get_unit_template(recording, sorting, unit_ids=unit_ids)
    else:
        templates = st.postprocessing_tools.get_unit_template(recording, sorting)
    snr_list = []
